...
Не стану расписывать, чтобы не терялось время на чтение, по сути процесс аналогичен первому. 
В macrotasks выполнился первый обработчик, но все еще висит второй и начинается его исполнение 
=> обработка синхронного кода из второй анонимной функции 
=> обработка микротасков 
=> дальнейшая обработка макростасков если они есть => очистка коллстека и очередей
...
Почему же у нас отличается результат в консоле?
 А потому что в первом случае после клика в очереди macrotasks queue мы имели две отдельный задачи,
соответственно когда код из первой выполнялся, eventloop перед выполнением второй мог посмотреть 
на очереди макро- и микро- тасков и выбрать что ему выполнить. Во втором же примере в callstack попадает функция button.click(). 
  
Далее вызывается код из первого обработчика, выполняет синхронный код из него, добавляются новые задачи в макро- и микро- очереди. 
И что важно в callstack'е до сих пор висит button.click(). Соответственно (eventloop не проверяет что ему выбрать макро- или микро- 
ведь коллстек не пуст) вызывается код из второго обработчика и исполняется его синхронный код (+ добавление в очереди каких-то задач). 

И только после того как весь синхронный код выполнен, стек очищается, event loop может заглянуть в макро- и микро- очереди 
и посмотреть что в них. Если там есть микро-задачи, он выполнит в первую очередь их и затем перейдет к макро задачам. 
То есть если бы в теле двух обработчиков были какие-то макро-таски (например сет-таймауты, даже с 0ms "хотя по факту там 4+ ms"),
то они выполнились бы после наших промиссов, например:
